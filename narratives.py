...
def generate_compelling_hook(trade, signals, sensitivity):
    ...
    if has_fresh_wallets and has_clustering:
        hook = (
            f"Polymarket is pricing a {price_percent:.1f}% chance of {market[:60]}...\n\n"
        ...
        hook = (
            f"Polymarket is pricing a {price_percent:.1f}% chance of {market[:60]}...\n\n"
            f"{signal_count} suspicious signals just fired.\n\n"
            "That's not random."
        )
...
def generate_twitter_thread(alert_data):
    ...
    thread.append(
        ...
        f"• Wallet: {wallet[:15]}...\n\n"
        "Pattern suggests insider information."
    )
    ...
    thread.append(
        ...
        "We're tracking this wallet and market movement.\n\n"
...
def generate_youtube_short_hook(alert_data):
    ...
    """
    trade = alert_data.get("trade", {})
    signals = alert_data.get("signals", [])  # FIXED: [] not [})
    market = trade.get("market_question", "Unknown Market")
    price = trade.get("price", 0)
    amount = trade.get("usd", 0)
